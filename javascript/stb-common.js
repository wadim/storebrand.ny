  ...
  /*
   * Common functions for general use on storebrand.no
  /*
  ...
  function scrollToHash(hash) {
    ...
    if (target.length) {
      ...
      // Check if the navigation header is fixed. If yes, set the scrollto position by subtracting the height of the fixed header
      if($('.navbar-fixed-top').css("display") == "block") {
        scrollToPosition -= $('.navbar-fixed-top').height();
      }
      $('html,body').animate({ scrollTop: scrollToPosition }, 1000);
      // Highlight the clicked heading for a brief period to make it easier to see what just happened.
...
/* ***** End of functionality related to login ***** */

/* Basic check, visitor notified about cookie use */
function acceptCookies(){
  createCookie('acceptCookies','Yes', '90');
  $('#cookieNote').hide('slow');
}
/* End of cookie use acceptance check */
